...
import {
    ...
    createSchemaTestMultilayerNetwork,
    createFullTestMultilayerNetwork,
    invalidIds,
    createActorsTestMultilayerNetwork
} from "../testNetwork";
...
describe("MultilayerNetwork", () => {
    ...
    describe("getLinksCount", () => {
        ...
        it("ok - network is created by layers", () => {
            ...
            const layers: Record<LayerId, { partitionsIds: PartitionId }> = {
    ...
    describe("iterate", () => {
        ...
        it("check actors", () => {
            class TestMultilayerNetwork extends MultilayerNetwork
            {
                ...
                {
                    const network: TestMultilayerNetwork = createActorsTestMultilayerNetwork(testMultilayerNetwork);
        ...
        it("check links", () => {
            class TestMultilayerNetwork extends MultilayerNetwork
            {
                public static iterate(): void
                {
                    const network: TestMultilayerNetwork = createFullTestMultilayerNetwork(testMultilayerNetwork);
                    ...
                    network.iterate({
                         callback: ({ links }) => {
                            for(const layerId in testMultilayerNetwork.data!.links)
                            {
                                const { partitionIds, directed } = network.getLayerSchema({ layerId });
                                const layer = (
                                    (partitionIds.source == partitionIds.target && !directed) ?
                                        links.get(layerId)
                                   :(directed) ?
                                        (links.get(layerId)! as any).out
                                   :
                                        (links.get(layerId)! as any).source
                    ...
                    network.iterate({
                        callback: ({ validators }) => {
                            ...
                            for(const layerId in testMultilayerNetwork.schema!.layers)
                            {
                                ...
                                const { weighted } = (testMultilayerNetwork.schema!.layers as any)[layerId];
                                if(weighted)
                                {
