...
import { MultiplexAlreadyExistingActorError, MultiplexAlreadyExistingLayerError, MultiplexAlreadyExistingLinkError, MultiplexNetwork, MultiplexNonExistingActorError, MultiplexNonExistingLayerError, MultiplexNonExistingLinkError, MultiplexNonWeightedLayerError, MultiplexWeightedLayerError } from "@/core";
import { createSchemaTestMultiplexNetwork, createActorsTestMultiplexNetwork, createFullTestMultiplexNetwork, testMultiplexNetwork } from "../testNetwork";
...
describe("MultiplexNetwork", () => {
    ...
    describe("getLayerSchema", () => {
        it("ok", () => {
            const network = createSchemaTestMultiplexNetwork(testMultiplexNetwork);
            for (const layerId in testMultiplexNetwork.schema) {
                const schema = testMultiplexNetwork.schema[layerId];
                        ...
                        callback: ({ links }) => {
                            for (const layerId in testMultiplexNetwork.data.links) {
                                ...
                                const layer = (nonSpecificLayer instanceof Map
                                    ? links.get(layerId)
                                    : ((nonSpecificLayer.in !== undefined)
                                        ? nonSpecificLayer.out
                                        : nonSpecificLayer.source));
                                let linksCount = 0;
                                for (const { sourceActorId, targetActorId } of testMultiplexNetwork.data.links[layerId]) {
                ...
                static iterate() {
                    ...
                    network.iterate({
                        callback: ({ weights }) => {
                            for (const layerId in testMultiplexNetwork.data.links) {
                                for (const { sourceActorId, targetActorId, weight } of testMultiplexNetwork.data.links[layerId]) {
                                    if (testMultiplexNetwork.schema[layerId].weighted) {
                        ...
                        callback: ({ validators }) => {
                            ...
                            for (const layerId in testMultiplexNetwork.schema) {
                                ...
                                expect(() => validators.schema.validateLayerIfNotExists({ layerId }))
                                    .toThrow(MultiplexAlreadyExistingLayerError);
                                const { weighted } = testMultiplexNetwork.schema[layerId];
                                if (weighted) {
