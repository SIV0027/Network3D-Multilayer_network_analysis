...
import {
    SingleLayerAlreadyExistingActorError,
    SingleLayerAlreadyExistingLinkError,
    SingleLayerNetwork,
    SingleLayerNonExistingActorError,
    SingleLayerNonExistingLinkError,
    SingleLayerNonWeightedLayerError,
    SingleLayerWeightedLayerError
} from "@/core";

import {
    testSingleLayerNetwork,
    createActorsTestSingleLayerNetwork,
    createSchemaTestSingleLayerNetwork,
...
describe("SingleLayerNetwork", () => {
    ...
    describe("iterate", () => {
        it("check actors", () => {
            class TestSingleLayerNetwork extends SingleLayerNetwork
            {
                ...
                {
                    const network: TestSingleLayerNetwork = createFullTestSingleLayerNetwork(testSingleLayerNetwork);
                ...
                public static iterate(): void
                {
                    ...
                    network.iterate({
                        callback: ({ links }) => {                            
                            for(const { sourceActorId, targetActorId } of testSingleLayerNetwork.data!.links)
                            {
                                expect((links as any).out.get(sourceActorId)!.has(targetActorId)).toBe(true);
                            }

                            let linksCount = 0;
                            for(const [_, neighbours] of (links as any).out)
                            {
    ...
    describe("constructor", () => {
        ...
        it("ok", () => {
            let network: SingleLayerNetwork;
